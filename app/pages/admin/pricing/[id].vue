...
const { data: p } = await useFetch(`/api/pricing/${id}`)
const { data: pricing } = await useFetch("/api/pricing")

const form = reactive({
  label: p.value?.label ?? "",
  firstDog: Number(p.value?.firstDog ?? 0),
  secondDog: Number(p.value?.secondDog ?? 0),
  perDog: Number(p.value?.perDog ?? 0),
  position: p.value?.position ?? 1,
})
const loading = ref(false)
const deleteTarget = ref(false)
const deleting = ref(false)

const maxPosition = computed(() => pricing.value?.length ?? 1)

async function submit() {
  loading.value = true
  try {
    await $fetch(`/api/pricing/${id}`, { method: "PUT", body: form })
...
<template>
  <header :style="{ marginBottom: '24px' }">
    ...
    <h1 :style="{ fontFamily: 'var(--font-display)', fontSize: '32px', fontWeight: '500', marginTop: '12px' }">Modifier le tarif</h1>
    <p :style="{ marginTop: '6px', color: 'var(--ink-2)', fontSize: '14px' }">Édition de la formule « {{ p?.label }} »</p>
  </header>
  ...
  <AdminDeleteModal v-if="deleteTarget" :label="p?.label ?? ''" :loading="deleting" @confirm="confirmDelete" @cancel="deleteTarget = false" />
</template>
